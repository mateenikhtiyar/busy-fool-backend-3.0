...
let IngredientsService = class IngredientsService {
    ...
    requiredFields = ['name', 'unit', 'quantity'];
    fieldTypes = {
        name: 'string',
        unit: 'string',
        quantity: 'number',
    ...
    constructor(ingredientRepository, stockRepository, productIngredientRepository, purchaseRepository, usersService) {
     ...
     */
    async create(createIngredientDto, userId) {
        ...
        if (createIngredientDto.waste_percent < 0 ||
        ...
        const { cost_per_ml, cost_per_gram, cost_per_unit } = this.calculateCosts(createIngredientDto.purchase_price, createIngredientDto.waste_percent, createIngredientDto.unit, createIngredientDto.quantity);
    ...
    async bulkCreate(createIngredientDtos, userId) {
        const user = await this.usersService.findById(userId);
        if (!user)
            throw new common_1.BadRequestException('User not found');
        if (!createIngredientDtos.length) {
            throw new common_1.BadRequestException('No ingredients provided');
        }
        const ingredients = [];
        for (const dto of createIngredientDtos) {
            ...
            if (existing)
                throw new common_1.BadRequestException(`Ingredient '${dto.name}' already exists`);
            ...
            await this.stockRepository.save(stock);
            const { cost_per_ml, cost_per_gram, cost_per_unit } = this.calculateCosts(dto.purchase_price, dto.waste_percent, dto.unit, dto.quantity);
            savedIngredient.cost_per_ml = cost_per_ml ?? null;
            savedIngredient.cost_per_gram = cost_per_gram ?? null;
            savedIngredient.cost_per_unit = cost_per_unit ?? null;
    ...
    async importCsv(file, userId) {
        ...
        if (missingRequired.length > 0) {
            throw new common_1.BadRequestException(`Missing required fields in CSV: ${missingRequired.join(', ')}. Ensure your CSV includes columns for ${this.requiredFields.join(', ')}.`);
        }
        const results = [];
        const errors = [];
        let dataRows = 0;
        ...
        for await (const row of parser) {
            ...
            try {
                ...
                if (existing)
                    throw new Error(`Ingredient '${row[autoMapping['name']]}' already exists`);
                const createDto = {
                ...
                await this.stockRepository.save(stock);
                const { cost_per_ml, cost_per_gram, cost_per_unit } = this.calculateCosts(createDto.purchase_price, createDto.waste_percent, createDto.unit, createDto.quantity);
                savedIngredient.cost_per_ml = cost_per_ml ?? null;
                savedIngredient.cost_per_gram = cost_per_gram ?? null;
                savedIngredient.cost_per_unit = cost_per_unit ?? null;
    ...
    async findOne(id, userId) {
        ...
        if (!ingredient) {
            throw new common_1.NotFoundException(`Ingredient with ID ${id} not found for this user`);
        }
        return ingredient;
    }
    /**
     ...
     * @throws NotFoundException if ingredient is not found for this user
     */
    async update(id, updateIngredientDto, userId) {
        const ingredient = await this.ingredientRepository.findOne({
            where: { id, user: { id: userId } },
        });
        if (!ingredient) {
        ...
        if (updateIngredientDto.waste_percent &&
        ...
        const totalUsed = (await this.stockRepository.sum('remaining_quantity', {
            ingredient: { id },
        })) || 0;
        if (newQuantity < totalUsed) {
            throw new common_1.BadRequestException(`New quantity (${newQuantity}) cannot be less than used stock (${totalUsed})`);
        }
        const { cost_per_ml, cost_per_gram, cost_per_unit } = this.calculateCosts(newPurchasePrice, newWastePercent, newUnit, newQuantity);
    ...
    async remove(id, userId) {
        ...
        if (ingredient.productIngredients &&
            ingredient.productIngredients.length > 0) {
            await this.productIngredientRepository.remove(ingredient.productIngredients);
        }
    ...
    calculateSimilarity(s1, s2) {
        ...
        for (let i = 0; i < tokens1.length; i++) {
            for (let j = 0; j < tokens2.length; j++) {
                if (tokens1[i] === tokens2[j]) {
                    const weight = i === tokens1.length - 1 || j === tokens2.length - 1
                        ? lastTokenWeight
                        : 1;
        ...
        const maxTokens = Math.max(tokens1.reduce((sum, t, i) => sum +
            (i === tokens1.length - 1 ? lastTokenWeight : 1) *
                (weightMap[t] || 1), 0), tokens2.reduce((sum, t, i) => sum +
            (i === tokens2.length - 1 ? lastTokenWeight : 1) *
                (weightMap[t] || 1), 0));
        return maxTokens ? commonTokens / maxTokens : 0;
    }
    isValidMapping(header, field) {
        ...
        return (!(fieldType === 'number' &&
            !headerTokens.some((t) => ['price', 'cost', 'percent', 'quantity', 'stock'].includes(t))) &&
            !(fieldType === 'string' &&
                headerTokens.some((t) => ['price', 'cost', 'percent', 'quantity', 'stock'].includes(t))));
    }
    ...
    generateAutoMapping(headers, suggestedMappings) {
        const autoMapping = {};
        for (const header of headers) {
    ...
    calculateCosts(purchase_price, waste_percent, unit, quantity) {
        ...
        if (isLiters)
            totalQuantity *= 1000; // Convert L to ml
        if (isKilograms)
            totalQuantity *= 1000; // Convert kg to g
        if (isMilliliters || isLiters)
            isMilliliters = true;
        if (isGrams || isKilograms)
            isGrams = true;
